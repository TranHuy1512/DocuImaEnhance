...
def main():
    ...
    # # # #
        ...
        ######################### Compute Importance
        if  opt['train']['ComputeImportance'] == True:
            logger.info('Compute Importance Params')
            for batch_step, train_data in enumerate(train_loader):
                model.feed_data(train_data)
        ...
        if opt['datasets'].get('val', None) and epoch % opt['train']['val_epoch'] == 0:
            ...
            print('validation loader长度:', len(val_loader))
            for val_data in val_loader:
                ...
                logger.info('id:{} img mean:{} gt mean:{}'.format(idx, np.mean(en_img), np.mean(gt_img)))
                if math.isinf(psnr_inst) or math.isnan(psnr_inst):
                    psnr_inst = 0
                    idx -= 1
                else:
                    avg_psnr_all = avg_psnr_all + psnr_inst
                if idx % 100 == 0:
                    print("Test the {} the image".format(idx))
                # pbar.update('Test {}'.format(img_name))
                    
            # log
            logger.info('# Validation # PSNR: {:.4e}, '.
                        format(avg_psnr_all / idx))
            logger.info(
        ...
        if epoch % opt['logger']['save_checkpoint_epoch'] == 0 and epoch >= 0:
            if rank <= 0:
