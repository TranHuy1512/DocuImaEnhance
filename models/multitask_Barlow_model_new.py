...
class BarlowTwins(nn.Module):
    def __init__(self):
        ...
        for i in range(len(sizes) - 2):
            layers.append(nn.Conv2d(int(start_dim/(pow(2,i))), int(start_dim/(pow(2,i+1))), kernel_size=3, stride=1, padding=1, bias=False))
    ...
    def forward(self, y1):
        ...
        #z1 = torch.mean(z1, 1).squeeze().view(z1.shape[0],-1)
        #z2 = torch.mean(z2, 1).squeeze().view(z2.shape[0],-1)
...
class Barlow_Loss(nn.Module):
    ...
    def forward(self, instance, ins1, ins2, ins3, ins4):
        con_loss = self.latent_contrast(instance, ins1, ins2, ins3, ins4)
    ...
    def latent_contrast(self, instance, ins1, ins2, ins3, ins4):
        # asymmetric
        '''
        con_loss = (self.cor_mat(instance, ins1) +
                   self.cor_mat(instance, ins2) +
                   self.cor_mat(instance, ins3) +
                   self.cor_mat(instance, ins4))/4
        '''
        # symmetric
        con_loss = (self.cor_mat(instance, ins1) +
                   self.cor_mat(instance, ins2) +
                   self.cor_mat(instance, ins3) +
                   self.cor_mat(instance, ins4) +
                   self.cor_mat(ins1, ins2) +
                   self.cor_mat(ins1, ins3) +
                   self.cor_mat(ins1, ins4) +
                   self.cor_mat(ins2, ins3) +
                   self.cor_mat(ins2, ins4) +
                   self.cor_mat(ins3, ins4) )/10
        return con_loss
    
########################################################

class multitask_Barlow_model(BaseModel):
    def __init__(self, opt):
        super(multitask_Barlow_model, self).__init__(opt)
        ...
        print(self.device)
        #######
        self.netG = SeeInDark().to(self.device)
        self.projector = BarlowTwins().to(self.device)
        #######
        # self.netG = doc_Barlow().to(self.device)
        if opt['dist']:
...
####################################################################
        if self.is_train:
            ...
            if loss_type == 'l1':
                ...
                self.barlow = Barlow_Loss().to(self.device)        
            else:
    ...
    def optimize_parameters(self, step):
        ...
        self.netG.cuda()
        # forward to get variable
        out,instf,fusef = self.netG(self.Back)
        out_blur,inst_blur,fuse_blur = self.netG(self.Blur)
        out_noise,inst_noise,fuse_noise = self.netG(self.Noise)
        out_watermark,inst_watermark,fuse_watermark = self.netG(self.Watermark)
        out_shadow,inst_shadow,fuse_shadow = self.netG(self.Shadow)
        # projecton
        instance = self.projector(instf)
        inst1 = self.projector(inst_blur)
        inst2= self.projector(inst_noise)
        inst3 = self.projector(inst_watermark)
        inst4 = self.projector(inst_shadow)
        ...
        if self.opt['train']['distill']:
            ...
            with torch.no_grad():
                self.netG_Pre.eval()
                _,gtinstf,gtfusef,_ = self.netG_Pre(var_fake.detach())
        '''
        ...
        barlow_loss = self.barlow(instance, inst1, inst2, inst3, inst4)
        mse_loss = self.mse(out, gt)
        bce_loss = self.cri_bce(out, gt)
        print('mse:', mse_loss, 'bce', bce_loss, 'l_constrat type', l_contrast, 'barlow loss', barlow_loss)
        
        # Original
        # l_total =  mse_loss + 5*bce_loss + l_contrast + 1/30000*barlow_loss
        # Ablation
        # 0725 l_total =  mse_loss + 5*bce_loss + 1/30000*barlow_loss
        # 0727 l_total =  mse_loss + l_contrast + 1/30000*barlow_loss
        l_total =  5*bce_loss + l_contrast + 1/30000*barlow_loss

        # print('mse:', self.mse(out, gt), 'bce:', self.cri_bce(out, gt),'con:', l_contrast)

                  # + 1.2*self.cri_pix(out, out_pre)
        # Distill Loss
        # We don not need it currently
        '''
        if self.opt['train']['distill']:
    ...
    def val(self):
        ...
        with torch.no_grad():
            ...
            for l in range(test_image_p.shape[0]):
                ...
                # self.generator(tmp)
                ##
                predict = self.netG(tmp.to(self.device))[0].squeeze().permute((1,2,0)).cpu().numpy()
